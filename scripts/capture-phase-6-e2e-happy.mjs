...
import { redactE2EArtifact } from "./lib/e2e-artifact-redaction.mjs";
...
const fixtureToken = process.env.PHASE6_E2E_FIXTURE_TOKEN?.trim()
  || (requestedUrl ? null : randomBytes(24).toString("base64url"));
...
try {
  await waitForServer();
  const sessionResponse = await fetch(`${origin}/api/test/fixture-session`, {
    method: "POST",
    headers: { authorization: `Bearer ${fixtureToken}` },
    redirect: "error",
  });
  assert.equal(sessionResponse.status, 204, "fixture session bootstrap must return 204");
  const setCookieHeader = sessionResponse.headers.get("set-cookie") ?? "";
  const fixtureCookieValue = setCookieHeader
    .split(";")[0]
    ?.split("=")
    .slice(1)
    .join("=");
  assert.ok(fixtureCookieValue, "fixture session cookie must be issued");
  ...
  const cookieResult = await client.send("Network.setCookie", {
    name: "jipbab-phase6-fixture",
    value: fixtureCookieValue,
    url: origin,
    httpOnly: true,
    secure: origin.startsWith("https://"),
    sameSite: "Strict",
  });
  assert.equal(cookieResult.success, true, "fixture session cookie must be installed");
  await client.send("Emulation.setDeviceMetricsOverride", {
    width: 390,
    height: 844,
    deviceScaleFactor: 1,
  ...
  await evaluate(client, `() => {
    localStorage.setItem('jipbab:e2e-fixture', 'true');
    return true;
  }`);
  ...
  writeFileSync(artifactPaths.network, `${JSON.stringify(redactE2EArtifact(networkLog), null, 2)}\n`);
  writeFileSync(artifactPaths.console, `${JSON.stringify(redactE2EArtifact(consoleLog), null, 2)}\n`);
  writeFileSync(artifactPaths.accessibility, `${JSON.stringify(redactE2EArtifact(accessibilitySummary), null, 2)}\n`);
  writeFileSync(artifactPaths.result, `${JSON.stringify(redactE2EArtifact({
