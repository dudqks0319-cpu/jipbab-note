...
- Phase 0/1/2와 Phase 7 feedback, FE-008 serving, FE-014 progress DB rollout: publication/auth migrations, Phase 1 schema/catalog, `20260710160000_add_distributed_api_rate_limits.sql`, `20260714100000_add_recipe_feedback.sql`, `20260714110000_extend_recipe_feedback_completion_details.sql`, `20260715100000_add_recipe_serving_variants.sql`, `20260715110000_add_recipe_progress.sql`은 로컬 검증만 완료됐고 운영에는 미적용이다. 현재 운영의 미검수 레시피 노출과 device-header 권한 경로는 앱/DB 동시 rollout 전까지 남으며, capture/restore, 분산 RPC, 비공개 feedback·progress 권한, 검수된 인분별 수량과 progress 충돌의 staging PostgreSQL 실행 증거가 아직 없다.
...
- staging에 `supabase/migrations/20260710130000_gate_recipe_publication.sql`, `supabase/migrations/20260710140000_replace_device_guest_auth_with_signed_sessions.sql`, `supabase/migrations/20260710150000_add_recipe_v2_schema_and_versioning.sql`, `supabase/migrations/20260710151000_seed_phase1_ingredient_catalog.sql`, `supabase/migrations/20260710160000_add_distributed_api_rate_limits.sql`, `supabase/migrations/20260714100000_add_recipe_feedback.sql`, `supabase/migrations/20260714110000_extend_recipe_feedback_completion_details.sql`, `supabase/migrations/20260715100000_add_recipe_serving_variants.sql`, `supabase/migrations/20260715110000_add_recipe_progress.sql`을 순서대로 적용합니다.
- 각 공개 후보에 기준 인분을 포함한 2개 이상의 `serving_variants`를 편집자 검수값으로 입력합니다. 수량, 단위, 도구, 시간 안내는 이름이나 배수 계산으로 생성하지 않습니다. 빈 배열이나 불완전한 변형이 있는 레시피는 목록과 상세 모두에서 숨겨지는지 확인합니다.
...
- staging 서버에 32자 이상의 server-only `API_RATE_LIMIT_HMAC_SECRET`을 설정하고 목록·상세·추천 API의 정상, `429`, `503`, 잘못된 입력 경로를 검증합니다. `POST /api/v1/recipe-feedback`은 정상 `201`, 멱등 `200`, 무서명 `401`, 본문 `400`·`413`, 제한 `429`, 저장소 미준비 `503`, app role 직접 테이블 접근 거부를 확인합니다. `GET|POST /api/v1/recipe-progress`는 영구 사용자 생성 `201`·조회/수정 `200`, 무서명·익명 `401`, 오래된 기준 시각과 생성 경쟁 `409`, 본문 `400`·`413`, 제한 `429`, 저장소 미준비 `503`, 사용자 간 격리와 app role 직접 접근 거부를 확인합니다. `POST /api/v1/shopping/items/from-recipe`는 서명 세션의 새 항목 `201`·기존 별칭/수량 병합 `200`, 무서명 `401`, 레시피 밖 재료·검수되지 않은 인분 `400`, 본문 `413`, 제한 `429`, 저장소 미준비 `503`, 개인 범위와 교차 사용자 차단을 확인합니다. 자세한 계약은 `docs/api-v1-operations.md`를 따릅니다.
- 운영에는 migration history와 백업 확인 후 Phase 0 두 migration과 matching app build를 먼저 함께 적용합니다. Phase 1 두 migration, Phase 2 rate-limit migration, Phase 7 feedback migration, FE-008 serving migration과 검수 데이터, FE-014 progress migration은 staging 복원·권한·충돌 시험과 API cutover 계획이 승인된 뒤 별도 rollout합니다. serving migration만 먼저 적용하면 공개 목록이 의도적으로 fail-closed 될 수 있으므로 matching `serving_variants` 데이터와 앱을 같은 변경 창에서 전환합니다. progress API는 조리 화면의 사용자 충돌 선택 UI가 준비되기 전에 자동 동기화에 사용하지 않습니다. `NEXT_PUBLIC_SUPABASE_ANONYMOUS_AUTH_ENABLED`는 abuse controls가 준비될 때까지 `false`로 유지합니다.
