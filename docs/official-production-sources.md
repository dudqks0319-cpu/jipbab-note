...
| production hotfix code | `agent/sync-ux-release`의 `e12fd87ff4e8f0252356f9af05749caf27b13dc6` | 현재 운영 런타임 기준 코드 |
...
| 현재 production deployment | `dpl_F38f834w18FhLgfJtoFFj8PGA1jM` | `READY`, `e12fd87` 레시피 재료 안내 hotfix |
...
현재 production 런타임은 canonical 저장소의 `e12fd87ff4e8f0252356f9af05749caf27b13dc6` 제한 웹 hotfix와 일치한다. 다만 Vercel 프로젝트의 Git 연결은 여전히 오래된 private 저장소 `jipbab-note-app/main`이므로 다음 일반 배포는 Git integration만으로 재현할 수 없고 P0 source mismatch가 남는다.
...
2026-07-15 웹 hotfix에서는 운영 도메인과 Vercel 프로젝트를 유지한 채 `e12fd87ff4e8f0252356f9af05749caf27b13dc6` 산출물을 배포했다. GitHub default branch 이동과 Vercel Git relink는 수행하지 않았으며 다음 비-hotfix production 배포 전 해결해야 한다.
