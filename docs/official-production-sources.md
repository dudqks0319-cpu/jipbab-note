...
Status: `canonical Git repository connected; Draft PR #9 open; production SHA alignment open`
...
| 보호 브랜치 통합 | Draft PR [#9](https://github.com/dudqks0319-cpu/jipbab-note/pull/9), opening head `69f18cb` → `main` | 자동 merge·Production 승격 없이 검토 중 |
...
현재 production 런타임은 canonical 저장소의 `3a8f72f78cde6588be86795d6fd4da4dc9d6308f` 제한 웹 hotfix와 일치한다. Vercel Git 연결 자체는 public canonical 저장소로 교정된 상태지만, 보호된 GitHub 기본 브랜치 `main`(`86e2bc2`), 최신 검증 코드 후보 `01e9895`, production 런타임 `3a8f72f`가 서로 다르다. 재감사 시작 시 `agent/sync-ux-release`를 `main`으로 보내는 열린 PR이 없었고, 이후 누적 범위를 공개 검토하기 위한 Draft PR #9를 열었다. 따라서 P0는 저장소 연결 문제가 아니라 Draft PR의 검토·검사와 production 승격 SHA를 하나로 정하는 문제로 좁혀진다.
...
1. Draft PR #9의 required checks와 사람 리뷰를 통과시키고, 102개 커밋·2,586개 파일의 누적 범위를 보안·동기화·publication·UX·대량 자산 순서로 검토한다. 검토 전에는 Ready 전환이나 merge를 하지 않는다.
2. PR merge SHA를 최종 production 승격 후보로 고정하고 `01e9895`의 exact Preview 증거가 merge 결과에서도 유지되는지 재검증한다.
3. Draft PR #4는 `ux/home-today-action-v2` 기반이고 Toddler Meals CI가 실패 중이므로 PR #9와 분리한다.
