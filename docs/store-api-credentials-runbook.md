...
APP_STORE_CONNECT_BUILD_VERSION=2026062602
```
...
이 명령은 App Store Connect API에서 bundle `com.jipbab.note`, 현재 iOS build `2026062602`, processed build state, internal TestFlight beta group 존재 여부를 확인합니다.
...
- `target build not found`: App Store Connect에 현재 iOS build `2026062602`가 없거나 다른 앱 레코드를 보고 있습니다.
