...
- [ ] App Store만 먼저 진행할 경우 `pnpm release:capture-ios-real-device-qa` 실행: iOS CoreDevice, iOS archive/IPA, iOS 수동 QA 템플릿만 분리 캡처
- [ ] Play Store만 먼저 진행할 경우 `pnpm release:capture-android-real-device-qa` 실행: Android adb/package, Android AAB/APK, Android 수동 QA 템플릿만 분리 캡처
