...
- [ ] 데이터 보안 문항 입력: 이메일/앱 활동/기기 ID/카메라 사용 범위 확인
- [ ] 개인정보 처리방침 URL 및 지원 URL 입력
- [ ] 콘텐츠 등급 설문 완료
- [ ] 내부 테스트 트랙에 AAB 업로드
- [ ] Android 실기기에서 로그인, 바코드, 장보기 링크, 계정 삭제 요청 QA
...
| 카메라/바코드 QA | 기기 권한, 실제 바코드 인식률은 브라우저/빌드만으로 검증 불가 | 앱 운영자 | 권한 허용/거부, 스캔 성공, 수동 입력 fallback 확인 | 스토어 제출 전 |
