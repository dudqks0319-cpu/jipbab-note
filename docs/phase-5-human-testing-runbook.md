...
- `recipe_version`은 `docs/phase-5-human-test-packets.md`에 기록된 `phase5-sha256-*` 값을 사용한다. 조리 단계, 재료, 이미지, 출처 또는 안전 문구가 바뀌면 자동으로 새 버전이 되며 이전 증거를 재사용하지 않는다.
...
2. 레시피마다 `docs/phase-5-human-test-packets.md`의 결정적 콘텐츠 버전을 그대로 기록한다. 임의의 `v1`이나 날짜 버전을 만들지 않는다.
...
`docs/phase-5-actual-cooking-template.csv`의 최초 20개 행은 레시피별 대기 슬롯이다. `pnpm phase5:audit`가 현재 콘텐츠 버전을 미리 채우며, 사람이 입력한 증거 행은 덮어쓰지 않는다. 재시험은 기존 행을 지우지 않고 새 행으로 추가하며, 가장 최근 `completed_at`의 시도가 현재 판정이 된다.
...
첫 명령은 감사 문서, 콘텐츠 버전이 고정된 20개 실행 패킷, 빈 증거 템플릿을 생성한다. 호환되는 사람 기록은 덮어쓰지 않으며 현재 콘텐츠와 버전이 다른 기록이 있으면 자동 교체하지 않고 중단한다. 두 번째 명령은 추적된 패킷과 템플릿이 현재 코드와 같은지 확인한다. 마지막 명령은 현재 미완료 상태에서 의도적으로 non-zero와 `BLOCKED`를 반환한다.
