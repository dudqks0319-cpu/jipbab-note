...
const directDeleteRouteSource = readFileSync("app/api/account/delete/route.ts", "utf8");
...
test("server Supabase helper exposes a shared missing-config detector", () => {
  ...
  assert.match(serverSource, /export async function getAuthenticatedServerUser/);
  assert.match(serverSource, /client\.auth\.getUser\(accessToken\)/);
});
...
test("direct account deletion route returns generic errors without exposing server config", () => {
  assert.match(directDeleteRouteSource, /SERVICE_UNAVAILABLE_MESSAGE/);
  assert.match(directDeleteRouteSource, /계정 삭제 운영 설정을 확인 중입니다\. 잠시 후 다시 시도해 주세요/);
  assert.match(directDeleteRouteSource, /UNAUTHENTICATED_MESSAGE/);
  assert.match(directDeleteRouteSource, /INVALID_CONFIRMATION_MESSAGE/);
  assert.match(directDeleteRouteSource, /isMissingServerSupabaseConfigError/);
  assert.match(directDeleteRouteSource, /noStoreHeaders\(\)/);
  assert.doesNotMatch(directDeleteRouteSource, /SUPABASE_SERVICE_ROLE_KEY/);
  assert.doesNotMatch(directDeleteRouteSource, /deleteUser\(body/);
});
...
test("production account deletion smoke fails closed without leaking secrets", () => {
  ...
  assert.match(productionSmokeSource, /\/api\/account\/delete/);
  ...
  assert.match(productionSmokeSource, /EXPECTED_UNAUTHENTICATED_MESSAGE/);
  assert.match(productionSmokeSource, /response\.status === 403/);
  assert.match(productionSmokeSource, /directResponse\.status === 401/);
