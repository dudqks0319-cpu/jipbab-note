...
이미지 포함 최신 iOS build `2026062602`가 App Store Connect에 업로드되고 iOS 앱 버전 `1.0`에 연결된 뒤, 2026-06-27 10:15 KST에 App Review로 제출됐습니다. 2026-06-28 10:00 KST App Store Connect API 재확인에서 버전 상태가 `PENDING_DEVELOPER_RELEASE`로 바뀐 것을 확인했습니다. 즉 Apple 심사는 승인됐고, 수동 출시 요청만 남았습니다. 연결 빌드는 `2026062602`, 빌드 ID는 `9006b306-e08c-4091-829c-2934615be184`, 빌드 처리 상태는 `VALID`입니다.
...
QA 기준 App Store 제출 차단점은 해소됐습니다. 사용자가 TestFlight에서 Kakao 로그인 완료와 직접 계정삭제 완료를 확인했고, 계정삭제 완료 스크린샷 증거도 보존했습니다. 현재 iOS App Store 쪽은 공개 출시 요청만 남았습니다. 수동 출시가 선택되어 있으므로 개발자 출시 요청을 해야 실제 공개됩니다.
...
- Phase: `appstore_pending_developer_release`
...
App Store iOS는 승인됐고 개발자 수동 출시 요청만 남았습니다. 실제 공개 출시 요청은 되돌리기 어려운 publish 동작이므로 최종 확인 후 App Store Connect에서 진행합니다. Android/Play Console과 Cloudflare 전환은 별도 릴리스 범위로 남아 있습니다.
