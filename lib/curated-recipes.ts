...
type ReleaseRecipeDraft = {
  ...
  substituteIngredients?: Array<[name: string, display: string, beginnerNote?: string]>;
  safetyNotes?: string[];
...
function releaseRecipe(draft: ReleaseRecipeDraft): CuratedRecipe {
  return {
    ...
    substituteIngredients: (draft.substituteIngredients ?? []).map(([name, display, beginnerNote]) =>
      ingredient(name, display, beginnerNote, "원래 재료가 없을 때만 사용합니다.", false),
    ),
...
const RELEASE_RECIPE_30_ADDITIONS: CuratedRecipe[] = [
  ...
  releaseRecipe({
    id: "phase5-egg-fried-rice",
    name: "계란볶음밥",
    category: "밥",
    method: "볶기",
    calories: "520",
    thumbnailUrl: "/images/recipes/phase5-core/egg-fried-rice.png",
    ingredients: [
      ["밥", "1공기(210g)", "찬밥은 덩어리를 먼저 풀어두면 볶기 쉽습니다."],
      ["계란", "2개", "알레르기가 있으면 사용하지 않습니다."],
      ["대파", "2큰술", "송송 썰어 향을 냅니다.", "없으면 생략 가능합니다.", false],
      ["식용유", "1큰술", "계란과 밥이 팬에 붙지 않게 합니다."],
      ["간장", "1작은술", "팬 가장자리로 넣어 향만 더합니다."],
      ["소금", "1꼬집", "마지막에 맛을 보고 넣습니다.", "싱거울 때만 사용합니다.", false],
    ],
    cookingTime: 10,
    servings: 1,
    trustLabel: "10분 한끼",
    featuredReason: "계란과 밥을 따로 익혀 고슬하게 만드는 기본 볶음밥",
    beginnerSummary: "계란을 먼저 반쯤 익힌 뒤 밥을 넣고 주걱으로 눌러 풀면 뭉치지 않습니다.",
    imageCaption: "밥알 사이에 노란 계란 조각이 고르게 섞이고 팬 바닥에 물기가 없으면 완성입니다.",
    substituteIngredients: [["대파 대체", "양파 2큰술", "대파가 없을 때만 잘게 썬 양파를 사용합니다."]],
    safetyNotes: [
      "계란 껍데기가 조리 도구와 완성 음식에 닿지 않게 하고 손을 씻습니다.",
      "남은 볶음밥은 조리 후 2시간 안에 냉장하고 다음 날까지 중심부가 뜨거워지도록 재가열합니다.",
    ],
    steps: [
      ["계란 2개를 그릇에 풀고 찬밥 1공기는 주걱으로 덩어리를 나눕니다.", "밥이 딱딱하면 물 1작은술을 뿌려 30초만 데우세요.", "밥 덩어리가 엄지손톱 크기 이하로 풀리면 됩니다."],
      ["팬에 식용유 1큰술을 두르고 중불에서 30초 데운 뒤 계란을 넣어 40초 섞습니다.", "계란 표면이 촉촉할 때 다음 단계로 넘어갑니다.", "노란 계란이 작은 덩어리로 뭉치고 액체가 조금 남아 있으면 됩니다."],
      ["밥과 대파를 넣고 중불에서 3~4분, 주걱으로 누르고 뒤집으며 볶습니다.", "팬이 마르면 식용유 1작은술 대신 물 1큰술을 가장자리에 넣으세요.", "밥알이 하나씩 떨어지고 김이 고르게 나면 됩니다."],
      ["간장 1작은술을 팬 가장자리에 넣어 30초 섞고 맛을 본 뒤 소금으로 마무리합니다.", "간장은 밥 위에 직접 붓지 않아야 한곳만 짜지지 않습니다.", "팬 바닥에 물기가 없고 계란과 밥이 고르게 섞이면 완성입니다."],
    ],
  }),
  releaseRecipe({
    id: "phase5-soy-beef-bulgogi",
    name: "간장불고기",
    category: "메인",
    method: "볶기",
    calories: "430",
    thumbnailUrl: "/images/recipes/phase5-core/soy-beef-bulgogi.png",
    ingredients: [
      ...
      ["참기름", "1작은술", "불을 끈 뒤 넣습니다.", "없으면 생략 가능합니다.", false],
    ],
    cookingTime: 25,
    servings: 2,
    trustLabel: "팬 한 개",
    featuredReason: "닭고기 대체가 아닌 얇은 소고기로 만드는 정확한 간장불고기",
    beginnerSummary: "양념한 소고기를 넓게 펼쳐 익히고 붉은 부분이 없어질 때까지 볶습니다.",
    imageCaption: "소고기 안쪽에 붉은 부분이 없고 양파가 투명하며 팬에 맑은 양념이 조금 남으면 완성입니다.",
    substituteIngredients: [["설탕 대체", "갈은 배 2큰술", "설탕 대신 사용할 때 물은 1큰술 줄입니다."]],
    safetyNotes: [
      "생소고기와 접촉한 손·도마·칼·집게는 익힌 음식에 다시 사용하지 않습니다.",
      "가장 두꺼운 고기 안쪽의 붉은 부분과 핏빛 육즙이 없어야 하며 남은 음식은 2시간 안에 냉장합니다.",
    ],
    steps: [
      ["간장 3큰술, 물 3큰술, 설탕 1큰술, 다진 마늘 1작은술을 그릇에 섞습니다.", "설탕 알갱이가 보이지 않을 때까지 20초 저으세요.", "양념이 한 가지 갈색으로 고르게 섞이면 됩니다."],
      ["소고기 300g과 양파를 양념에 넣어 젓가락으로 풀고 냉장고에서 10분 둡니다.", "생고기를 만진 도구는 익힌 음식에 다시 쓰지 않습니다.", "고기 조각 사이에 양념이 묻고 서로 붙지 않으면 됩니다."],
      ["팬을 중강불로 1분 데운 뒤 고기를 한 겹으로 펼쳐 2분 익힙니다.", "고기를 한꺼번에 뭉쳐 놓으면 가운데가 익지 않습니다.", "팬에 닿은 면이 갈색으로 변하면 뒤집을 때입니다."],
      ["고기를 뒤집고 대파를 넣어 4~5분 볶아 붉은 부분을 완전히 없앱니다.", "가장 두꺼운 고기 한 점을 잘라 속을 확인하세요.", "고기 안쪽이 갈색이고 양파가 투명하면 됩니다."],
      ["불을 끄고 참기름 1작은술을 섞어 접시에 바로 옮깁니다.", "팬에 오래 두면 남은 열로 고기가 질겨집니다.", "맑은 양념이 고기에 얇게 묻고 핏빛 국물이 없으면 완성입니다."],
    ],
  }),
  releaseRecipe({
    id: "phase5-tteokbokki",
    name: "떡볶이",
    category: "간식",
    method: "끓이기",
    calories: "510",
    thumbnailUrl: "/images/recipes/phase5-core/tteokbokki.png",
    ingredients: [
      ...
      ["대파", "1/2대", "4cm 길이로 자릅니다.", "없으면 생략 가능합니다.", false],
    ],
    cookingTime: 18,
    servings: 2,
    trustLabel: "한 냄비 간식",
    featuredReason: "떡국이 아닌 고추장 양념에 조리는 기본 떡볶이",
    beginnerSummary: "떡이 말랑해지고 소스가 주걱 자국을 1초 남길 때 불을 끄면 식어도 지나치게 되직하지 않습니다.",
    imageCaption: "떡이 속까지 말랑하고 붉은 소스가 얇게 달라붙으며 냄비 바닥에 소스가 남아 있으면 완성입니다.",
    substituteIngredients: [["어묵 대체", "양배추 1줌", "어묵 알레르기가 있거나 없을 때 사용합니다."]],
    safetyNotes: [
      "떡은 질식 위험이 있으므로 영유아에게 제공하지 않고 성인도 한입 크기로 천천히 먹습니다.",
      "남은 떡볶이는 2시간 안에 냉장하고 다음 날 물을 더해 중심부까지 충분히 재가열합니다.",
    ],
    steps: [
      ["떡이 딱딱하면 찬물에 10분 담그고, 어묵과 대파를 한입 크기로 자릅니다.", "떡을 억지로 떼지 말고 물속에서 살살 분리하세요.", "떡이 서로 떨어지고 표면이 촉촉하면 됩니다."],
      ["냄비에 물 400ml, 고추장 2큰술, 설탕 1큰술, 간장 1큰술을 넣어 중불에서 2분 끓입니다.", "고추장 덩어리를 주걱으로 눌러 풀어주세요.", "국물이 붉은색으로 고르고 작은 거품이 올라오면 됩니다."],
      ["떡과 어묵을 넣고 중불에서 7~9분, 바닥을 긁으며 1분마다 섞습니다.", "바닥에 붙기 시작하면 불을 약하게 낮추고 물 2큰술을 넣으세요.", "떡을 반으로 잘랐을 때 단단한 흰 심이 없으면 익었습니다."],
      ["대파를 넣고 약불에서 1분 더 끓인 뒤 소스가 너무 되기 전에 불을 끕니다.", "식으면서 더 되직해지므로 냄비 바닥을 완전히 말리지 않습니다.", "주걱으로 바닥을 그었을 때 자국이 1초 보였다가 소스로 채워지면 완성입니다."],
    ],
  }),
  releaseRecipe({
    id: "phase5-chicken-breast-vegetable-stir-fry",
    name: "닭가슴살 채소볶음",
    category: "메인",
    method: "볶기",
    calories: "360",
    thumbnailUrl: "/images/recipes/phase5-core/chicken-breast-vegetable-stir-fry.png",
    ingredients: [
      ...
      ["다진 마늘", "1작은술", "향을 더합니다.", "없으면 생략 가능합니다.", false],
    ],
    cookingTime: 20,
    servings: 2,
    trustLabel: "단백질 반찬",
    featuredReason: "덮밥 대체가 아닌 닭가슴살과 여러 채소를 함께 볶는 메인 반찬",
    beginnerSummary: "닭가슴살을 먼저 완전히 익힌 뒤 채소를 넣어야 생닭 교차 오염과 덜 익힘을 줄일 수 있습니다.",
    imageCaption: "가장 큰 닭 조각을 잘랐을 때 속이 흰색이고 맑은 육즙이 나오며 채소 색이 선명하면 완성입니다.",
    substituteIngredients: [["애호박 대체", "브로콜리 1컵", "브로콜리는 한입 크기로 잘라 먼저 2분 데칩니다."]],
    safetyNotes: [
      "생닭과 접촉한 손·도마·칼·집게는 익힌 음식과 채소에 다시 사용하지 않습니다.",
      "가장 큰 닭 조각의 중심까지 흰색으로 익히고 남은 음식은 2시간 안에 냉장합니다.",
    ],
    steps: [
      ["닭가슴살은 2cm, 파프리카와 양파는 비슷한 크기, 애호박은 0.7cm 두께로 자릅니다.", "생닭용 도마와 칼은 채소에 다시 쓰지 말고 바로 세척하세요.", "닭과 채소 크기가 비슷하면 한입에 먹기 좋습니다."],
      ["팬에 식용유 1큰술을 두르고 중불로 1분 데운 뒤 닭을 한 겹으로 펼쳐 3분 익힙니다.", "처음 1분은 건드리지 않아야 팬에 덜 붙습니다.", "바닥 면이 흰색에서 연한 갈색으로 바뀌면 뒤집습니다."],
      ["닭을 뒤집고 물 2큰술을 넣어 뚜껑을 덮은 뒤 중약불에서 4분 익힙니다.", "가장 큰 조각을 잘라 분홍색이 남으면 2분 더 익히세요.", "닭 속이 완전히 흰색이고 맑은 육즙이 나오면 됩니다."],
      ["양파, 애호박, 파프리카, 다진 마늘을 넣고 중불에서 3~4분 볶습니다.", "채소를 너무 오래 익히면 물이 많이 나옵니다.", "양파 가장자리는 투명하고 파프리카 색은 선명하면 됩니다."],
      ["간장 1큰술을 팬 가장자리에 넣어 30초 섞고 가장 큰 닭 조각의 속을 다시 확인합니다.", "간장은 마지막에 넣어 채소에서 물이 많이 나오지 않게 합니다.", "닭 속에 분홍색이 없고 팬 바닥에 핏빛 국물이 없으면 완성입니다."],
    ],
  }),
];
