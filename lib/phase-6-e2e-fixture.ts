// 이 파일은 사람 검수 통계와 분리된 Phase 6 기술 E2E 전용 레시피를 안전하게 제공합니다.
import type {
...
export const PHASE6_E2E_FIXTURE_HEADER = "x-phase6-e2e-fixture";
...
export function shouldUsePhase6E2EFixture(
  ...
  if (env.APP_ENV !== "staging" && env.NODE_ENV !== "test") {
    return false;
  }
  if (env.PHASE6_E2E_FIXTURE_ENABLED !== "true") return false;
  const expected = env.PHASE6_E2E_FIXTURE_TOKEN?.trim() ?? "";
  const received = headers.get(PHASE6_E2E_FIXTURE_HEADER)?.trim() ?? "";
  return secureTokenMatches(received, expected);
}
