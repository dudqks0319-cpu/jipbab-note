...
export default function TodayActionCard({
  ...
  const reason = expiringMatchedIngredients.length > 0
    ? `${expiringMatchedIngredients.slice(0, 2).join(', ')} 먼저 쓰기 좋아요.`
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-[#2f2117] text-white shadow-[0_18px_36px_rgba(47,33,23,0.18)]">
      <div className="grid grid-cols-[1fr_116px] gap-3 p-4">
        <div className="min-w-0">
          <p className="text-[12px] font-black text-[#ffd8a8]">오늘 집밥 바로 시작</p>
        ...
        <Link
          ...
          className="relative block h-[116px] overflow-hidden rounded-[18px] bg-[#fff7ed]"
        >
      ...
      <div className="border-t border-white/10 px-4 pb-4">
        <div className="pt-3">
          <p className="text-[11px] font-black text-[#ffd8a8]">
            {expiringMatchedIngredients.length > 0 ? '오늘 먼저 쓸 재료' : '있는 재료로 시작'}
          </p>
        ...
        <div className="mt-3 grid grid-cols-[1.15fr_0.85fr] gap-2">
          <Link
            ...
            className="flex min-h-12 items-center justify-center rounded-[15px] bg-[#ea5a1f] px-3 text-[14px] font-black text-white shadow-[0_10px_20px_rgba(234,90,31,0.22)]"
          >
            {primaryCta.label}
          </Link>
          <Link
            href={secondaryCta.href}
            className="flex min-h-12 items-center justify-center gap-1 rounded-[15px] border border-white/15 bg-white/10 px-3 text-[12px] font-black text-[#ffe7c9]"
          >
            {missingCount === 1 ? (
              <>
